...
Trigger: the YouTube ingestion connector consumes externally shaped yt-dlp metadata and depends on yt-dlp's optional impersonation runtime. A translated `zh-Hans` track was selected for an English video because selection preferred Chinese before the video's original language.
...
- Bad: choose `zh-Hans` just because it appears first or because it is official when an English original track exists.

...
- [ ] 清空观察游标，不删除历史数据；请验收人发送一条 `/whoami`。
- [ ] 按同一时间窗口检查 adapter → pipeline → plugin → gateway → reply 五个边界。
...
- [ ] 人工发送 `/whoami`，确认收到内部用户编号。
- [ ] 再发送一次，确认编号稳定且数据库无重复身份。
- [ ] 重启 gateway 与 LangBot runtime 后复测一次。
- [ ] 检查 monitoring、bot/plugin 日志和任务证据均无私聊正文、昵称或 sender ID 明文。
- [x] 运行受影响的自动测试，并把命令、结果与已知限制写入本任务。

## 5. 完成门

- [ ] 由人工验收微信回复；自动检查不能替代真实微信收发。
- [ ] 更新父任务人工验收清单的对应证据，但不提前勾选 Telegram E2E。
